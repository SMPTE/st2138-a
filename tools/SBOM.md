...
| SBOM Generation Context | ⬜ | not emitted (`metadata.lifecycles`) |
...
### SBOM Generation Context — ⬜ not started
...
- For example, an SBOM generated from source code can be identified as
  ...
  `after build`.
- **Implementation:** Not emitted. Candidate: set `metadata.lifecycles` to
  `pre-build`, since the SBOM is generated from source descriptors.
